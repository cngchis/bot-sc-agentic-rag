...
def node_retrieve_qna(state: GraphState) -> GraphState:
    ...
    context, source, score = retrieve_context(state["query"])
    print(f"[Top Result] → {source} (Score: {score:.4f})")
    print(f'state: {state}')
    return {**state, "context": context, "source": source, "score": score}

def node_web_search(state: GraphState) -> GraphState:
    log_node("Web_Search", state)
    docs = web_search(state["query"], k=1)
    context = format_docs(docs)
    print(f'score: {state.get("score", 0.0):.4f}')
    return {**state, "context": context, "source": "Web Search"}

def node_check_relevance(state: GraphState) -> GraphState:
    score = state.get("score")

    print(f"[Check Relevance] → Score: {score}")
...
def node_chitchat(state: GraphState) -> GraphState:
    ...
    CHITCHAT_PROMPT = f"""
    Bạn là trợ lý ảo Techcombank. Trả lời ngắn gọn bằng tiếng Việt.
